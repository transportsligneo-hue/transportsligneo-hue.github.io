ALTER TABLE public.devis ADD COLUMN IF NOT EXISTS vehicules jsonb;
...
BEGIN
  ...
  v_multi := CASE WHEN jsonb_typeof(d.vehicules) = 'array' AND jsonb_array_length(d.vehicules) > 1 THEN d.vehicules ELSE NULL END;
  ...
  IF v_multi IS NOT NULL THEN v_is_ar := false; END IF;
  ...
  IF v_is_ar THEN
    ...
    END IF;
  ELSIF v_multi IS NOT NULL THEN
    v_aller := COALESCE(NULLIF((v_multi->0->>'prix'), '')::numeric, 0); v_retour := 0;
  ELSE
  ...
  -- Devis groupé : une mission par véhicule supplémentaire
  IF v_multi IS NOT NULL THEN
    FOR v_idx IN 1 .. jsonb_array_length(v_multi) - 1 LOOP
      v_line := v_multi -> v_idx;
      SELECT id INTO v_extra_id FROM public.missions
      WHERE devis_id = d.id AND leg_index = v_idx + 1 LIMIT 1;
      IF v_extra_id IS NULL THEN
        INSERT INTO public.missions (
          ...
          COALESCE(d.nom, ''), COALESCE(d.prenom, ''), COALESCE(d.email, ''), d.telephone,
          d.depart,
          COALESCE(NULLIF(trim(COALESCE(v_line->>'arrivee', '')), ''), d.arrivee),
          COALESCE(d.date_souhaitee, current_date), 'aller_simple',
          NULLIF(trim(COALESCE(v_line->>'marque', '')), ''),
          NULLIF(trim(COALESCE(v_line->>'modele', '')), ''),
          d.carburant,
          NULLIF(upper(trim(COALESCE(v_line->>'vin', ''))), ''),
          NULLIF(upper(trim(COALESCE(v_line->>'immatriculation', ''))), ''),
          d.message,
          COALESCE(NULLIF((v_line->>'prix'), '')::numeric, 0),
          v_status, v_group, 'simple', v_idx + 1,
          d.contact_depart_nom, d.contact_depart_tel, d.contact_depart_note,
          d.contact_arrivee_nom, d.contact_arrivee_tel, d.contact_arrivee_note
        ) RETURNING missions.id, missions.numero INTO v_extra_id, v_num;
        mission_id := v_extra_id; leg := 'vehicule_' || (v_idx + 1); numero := v_num; RETURN NEXT;
      END IF;
    END LOOP;
  END IF;
