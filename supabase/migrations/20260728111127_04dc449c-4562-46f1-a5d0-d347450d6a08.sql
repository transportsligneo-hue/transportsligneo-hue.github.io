CREATE OR REPLACE FUNCTION public.admin_convert_devis_to_missions(
  _devis_id uuid,
  _converted_by uuid DEFAULT NULL,
  _mission_status text DEFAULT 'en_attente'
)
...
BEGIN
  ...
  v_is_ar := (
    COALESCE(d.option_trajet, '') IN ('aller_retour', 'aller-retour')
...
BEGIN
  ...
  LOOP
    PERFORM * FROM public.admin_convert_devis_to_missions(r.id, NULL, 'en_attente');
  END LOOP;
END $$;