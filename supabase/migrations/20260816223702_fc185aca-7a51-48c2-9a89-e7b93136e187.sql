CREATE OR REPLACE FUNCTION public.admin_update_mission_infos(_trajet_id uuid, _patch jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.activity_logs (action, entity_type, entity_id, actor_user_id, metadata)
  ...
  -- destinataire client : uniquement s'il existe réellement dans auth.users
  SELECT p.id INTO client_uid
    FROM public.profiles p
    JOIN auth.users u ON u.id = p.id
   WHERE new_row.client_email IS NOT NULL AND lower(p.email) = lower(new_row.client_email)
   LIMIT 1;
