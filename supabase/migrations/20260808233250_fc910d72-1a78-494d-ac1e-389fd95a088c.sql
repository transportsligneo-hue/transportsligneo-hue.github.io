CREATE OR REPLACE FUNCTION public.admin_update_trajet_prix(
  _trajet_id uuid,
  _prix numeric,
  _prix_convoyeur numeric DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT prix, mission_group_id, leg_type, devis_id, mission_id
    INTO v_old, v_group, v_leg, v_devis, v_mission
...
REVOKE ALL ON FUNCTION public.admin_update_trajet_prix(uuid, numeric, numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_update_trajet_prix(uuid, numeric, numeric) TO authenticated;