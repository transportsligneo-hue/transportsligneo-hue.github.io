...
import { generateContratConvoyeurPdf, getLastContratPageCount } from "@/lib/documents-officiels";
...
export default function ContratYousignCard(props: Props) {
    ...
    try {
      ...
      const res = await sendContrat({
        data: {
          ...
          pdfBase64,
          pageCount: getLastContratPageCount(),
          snapshot: { siret, adresse, permis_numero: permis },
        },
      });
      toast.success(
        res.otpSms
          ? "Contrat envoyé pour signature (code SMS activé)."
          : "Contrat envoyé pour signature par email.",
      );
      await load();
    } catch (e) {
  ...
  const telecharger = async () => {
    ...
    setBusy(true);
    try {
      const { url } = await signedUrl({ data: { contratId: contrat.id } });
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-pro-text-soft">Signature électronique Yousign (eIDAS)</p>
          </div>
        </div>
        {ui && (
        ...
        <>
          {contrat && (
            <div className="mt-4 text-xs text-pro-text-soft space-y-1">
              ...
              {contrat.signed_at && <p>Signé le {new Date(contrat.signed_at).toLocaleString("fr-FR")}</p>}
          ...
          <p className="mt-3 text-[11px] text-pro-muted">
            Le contrat n'est jamais téléchargeable par le candidat avant signature : Yousign lui envoie
