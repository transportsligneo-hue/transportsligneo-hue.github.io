...
const Email = ({ prenom, numero, depart, arrivee, date, prix }: Props) => (
  <LigneoEmailShell
    preview="La mission vous est officiellement attribuée."
  ...
  >
    <SimpleCard
      ...
      subtitle={[numero ? `Mission ${numero}` : null, date].filter(Boolean).join(' · ')}
    />
    {prix ? <AmountRow label="Rémunération convoyeur" amount={`${prix} €`} /> : null}
  </LigneoEmailShell>
)
